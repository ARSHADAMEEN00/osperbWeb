...
export const createCODOrder = () => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post('/api/v1/order/cod/new', config)
...
// cancell order
export const cancellOrder = (id) => async (dispatch) => {
    try {

        dispatch({ type: CANCELL_ORDER_REQUEST })
        ...
        const { data } = await axios.put(`/api/v1/order/cancell/${id}`, config)

        dispatch({
            type: CANCELL_ORDER_SUCCESS,
            payload: data.success
        })

    } catch (error) {
        dispatch({
            type: CANCELL_ORDER_FAIL,
            payload: error.response.data.message
        })
    }
}
...
export const returnOrder = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`/api/v1/order/return/${id}`, config)
