...
export const getMatchingProductDetails = (id) => async (dispatch) => {
    try {

        dispatch({ type: MATCHING_PRODUCT_DETAILS_REQUEST })

        const { data } = await axios.get(`/api/v1/product/${id}`)
        
        dispatch({
            type: MATCHING_PRODUCT_DETAILS_SUCCESS,
            payload: data.product
        })

    } catch (error) {
        dispatch({
            type: MATCHING_PRODUCT_DETAILS_FAIL,
            payload: error.response.data.message
        })
    }
}
